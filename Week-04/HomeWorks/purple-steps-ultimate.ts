...
function clockify(){
  (function (){ // Drawing ClockFace
    for (let i=1, rotator = 0, count10 = 0, count1000=0,  cloner=0 ; i <=600; i++){
      drawLinedRects({startPosition: [canvas.width/2, canvas.height/2], cloneCount:2-cloner, sideSize:15, 
        ...
        rotator += 36; if (rotator === 361) rotator = 1;
      count10 += 1;
      switch (count10) {
        case 1:
          cloner = 1;
      ...
      count1000 += 0.6; if (count1000 === 1001) count1000 = 1;
    }
  }())
  let rotator = 0;
  let count10 = 0;
  let count1000=0;
  let cloner=0;
  setInterval(function(){ // Animating ClockParts
    drawLinedRects({startPosition: [canvas.width/2, canvas.height/2], cloneCount:2-cloner, sideSize:15, 
      cloneDegree:270 + count1000, rotationDegree:45 + rotator, fillColor:'violet',growthFactor:1.4});
    rotator += 72; if (rotator === 361) rotator = 1
    count10 += 1;
    switch (count10) {
      case 1:
        cloner = 1;
...
function drawLinedRects({ 
  ...
  let radius = Math.sqrt(2*(Math.pow((sideSize/2),2)));
  let rotationRadian = rotationDegree * Math.PI/180;
  let cloneRadian = cloneDegree * Math.PI/180;
  let startAngle = 2 * Math.PI / 8;
  let cloneDistance:number=0;
  let prevCloneDistance:number = (cloneDistanceMode === 'vertex') ? radius + cloneDistancePadding : /*side*/ sideSize/2 + cloneDistancePadding;
  let cloneMid = [midpoint[0], midpoint[1] ];
  let cloneRadius: number = 0; 
  ...
  for(let clones=0; clones<cloneCount; clones++){
    ...
    for (let side = 1; side <=4; side++){
      ctx.lineTo(
        cloneMid[0]  + cloneRadius * Math.cos(startAngle + rotationRadian + side * 2 * Math.PI / 4), 
        cloneMid[1]  + cloneRadius * Math.sin(startAngle + rotationRadian + side * 2 * Math.PI / 4)
        );
    }
